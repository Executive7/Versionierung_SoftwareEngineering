...
serviceRouter.post("/arbeitgeber", function(request, response) {
    ...
    if (helper.isUndefined(request.body.arbeit)) 
    ...
    const arbeitgeberDao = new ArbeitgeberDao(request.app.locals.dbConnection);
    try {
        var result = arbeitgeberDao.create(request.body.userID, request.body.arbeit, request.body.lohn, request.body.adresse, request.body.plz, request.body.stadt);
